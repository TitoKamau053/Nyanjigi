...
class SystemSettingsController {
  ...
  // Get all system settings (Admin only) - UPDATED with Auto-Init
  static async getAllSettings(req, res) {
    try {
      let settings = await SystemSettings.getAllSettings();
      
      // Auto-initialize if no settings found to fix empty page error
      if (!settings.all || settings.all.length === 0) {
          console.log('No settings found. Auto-initializing default settings...');
          await SystemSettings.initializeDefaultSettings();
          settings = await SystemSettings.getAllSettings(); // Fetch again
      }
  ...
  // Run system maintenance manually
  static async runSystemMaintenance(req, res) {
    try {
      const result = await SchedulerService.runSystemMaintenance();
  ...
  static async sendTestNotification(req, res) {
    try {
      ...
      const targetPhone = phone_number || process.env.ADMIN_PHONE || '0700000000';
      
      const result = await NotificationService.sendNotification(
        { id: 0, phone: targetPhone, full_name: 'System Admin' },
        'manual_sms', // Ensure you have a 'manual_sms' or similar template
        { message: 'Test notification from Nyanjigi System.' }
      );

      return ApiResponse.success(res, result, 'Test notification attempt completed');
    } catch (error) {
      return ApiResponse.error(res, error.message, 500);
    }
  }

  // Get security logs (using payment_logs as a proxy for now)
  static async getSecurityLogs(req, res) {
    try {
      const query = `
        SELECT id, 'Payment Log' as type, status, reason as details, timestamp as created_at 
        FROM payment_logs 
        ORDER BY timestamp DESC LIMIT 50
      `;
      const logs = await executeQuery(query);
      return ApiResponse.success(res, logs, 'Security logs retrieved successfully');
    } catch (error) {
      return ApiResponse.error(res, error.message, 500);
    }
  }
  ...
  static async updateContributionSettings(req, res) {
    try {
      ...
      if (Object.keys(updates).length === 0) return ApiResponse.error(res, 'No contribution settings provided to update', 400);

      const results = await SystemSettings.updateSettings(updates, req.admin.id);
      return ApiResponse.success(res, { updated_settings: results }, 'Contribution settings updated successfully');
    } catch (error) {
      return ApiResponse.error(res, error.message, 500);
    }
  }
  ...
  static async updateCompanySettings(req, res) {
    try {
      const { company_name, company_phone, company_email, company_address, logo_url } = req.body;
      const updates = {};
      ...
      if (Object.keys(updates).length === 0) return ApiResponse.error(res, 'No company settings provided to update', 400);

      const results = await SystemSettings.updateSettings(updates, req.admin.id);
      return ApiResponse.success(res, { updated_settings: results }, 'Company settings updated successfully');
    } catch (error) {
      return ApiResponse.error(res, error.message, 500);
    }
  }
  ...
  static async initializeSettings(req, res) {
    try {
      ...
      return ApiResponse.success(res, result, `Settings initialized successfully.`);
    } catch (error) {
      return ApiResponse.error(res, error.message, 500);
    }
  }
  ...
  static async resetSettings(req, res) {
    try {
      ...
      if (!confirm) return ApiResponse.error(res, 'Reset must be confirmed', 400);
      if (!category) return ApiResponse.error(res, 'Category is required', 400);

      const currentSettings = await SystemSettings.getSettingsByCategory(category);
      if (Object.keys(currentSettings).length === 0) return ApiResponse.notFound(res, 'No settings found for this category');

      await SystemSettings.initializeDefaultSettings();
      const updatedSettings = await SystemSettings.getSettingsByCategory(category);

      return ApiResponse.success(res, { category, reset_count: Object.keys(updatedSettings).length, settings: updatedSettings }, 'Settings reset successfully');
    } catch (error) {
      return ApiResponse.error(res, error.message, 500);
    }
  }
  ...
  static async exportSettings(req, res) {
    try {
      ...
      if (!include_sensitive) {
        ...
        for (const category of Object.values(exportData)) {
          for (const [key, setting] of Object.entries(category)) {
            if (sensitiveKeys.some(sensitive => key.includes(sensitive))) setting.value = '***hidden***';
          }
        }
      }
      ...
      return ApiResponse.success(res, { settings: exportData }, 'Settings exported successfully');
    } catch (error) {
      return ApiResponse.error(res, error.message, 500);
    }
  }

  // Get system status based on settings
  static async getSystemStatus(req, res) {
    try {
      const validatedSettings = await SystemSettings.getSettingsWithValidation();
      const criticalSettings = ['jenga_consumer_key', 'jenga_consumer_secret', 'stk_push_shortcode'];
      ...
      return ApiResponse.success(res, systemStatus, 'System status retrieved', systemStatus.status === 'healthy' ? 200 : 207);
    } catch (error) {
      return ApiResponse.error(res, error.message, 500);
    }
  }
  ...
  static async getSettingsByCategory(req, res) {
    try {
      ...
      if (Object.keys(settings).length === 0) return ApiResponse.notFound(res, 'No settings found for this category');
  ...
  static async getSetting(req, res) {
    try {
      ...
      if (value === null) return ApiResponse.notFound(res, 'Setting not found');
  ...
  static async updateSetting(req, res) {
    try {
      ...
      if (value === undefined || value === null) return ApiResponse.error(res, 'Setting value is required', 400);

      const isValid = SystemSettings.validateSetting(key, value);
      if (!isValid) return ApiResponse.error(res, 'Invalid setting value', 400);

      const updatedSetting = await SystemSettings.setSetting(key, value, description, category || 'general', req.admin.id);
  ...
  static async updateBillingSettings(req, res) {
    try {
      ...
      if (Object.keys(updates).length === 0) return ApiResponse.error(res, 'No billing settings provided to update', 400);

      const results = await SystemSettings.updateSettings(updates, req.admin.id);
      return ApiResponse.success(res, { updated_settings: results }, 'Billing settings updated successfully');
    } catch (error) {
      return ApiResponse.error(res, error.message, 500);
    }
  }
